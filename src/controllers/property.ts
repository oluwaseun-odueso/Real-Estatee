...
export async function getManyProperties(req: Request, res: Response) {
  try {
    const queries: QueryParamI = {
      page: Number(req.params.page) | 1,
      limit: Number(req.params.limit) | 20,
    };

    const manyMany = getAllProperties(queries);
    res.status(200).send(manyMany);
  } catch (error) {}
}
...
export async function addProperty(req: Request, res: Response) {
  try {
    ...
    res.status(201).send({
      success: true,
      message: "You have successfully put up a new property for sale",
      property,
    });
  } catch (error: any) {
...
export async function getProperty(req: Request, res: Response) {
  try {
    ...
    const propertyDetails = await getFullPropertyDetails(
      property_id,
      property.address_id
    );
    res.status(200).send({
...
export async function updateProperty(req: Request, res: Response) {
  try {
    ...
    const new_details = await getFullPropertyDetails(
      property_id,
      property.address_id
    );
