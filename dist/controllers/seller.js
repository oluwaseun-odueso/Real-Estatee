...
async function loginSeller(req, res) {
    try {
        ...
        if (await (0, sellerFunctions_1.confirmRetrievedPassword)(password, collectedUserPassword) !== true) {
...
async function uploadSellerImage(req, res) {
    try {
        ...
        const file = req.file;
        const result = await (0, s3_1.uploadFile)(file);
        await unlinkFile(file.path);
        await (0, sellerFunctions_1.saveSellerImageKey)(req.seller.id, result.Key);
        res.status(200).send({
            success: true,
